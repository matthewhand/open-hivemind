  ...
  return (
    <div className={`w-full ${className}`}>
      <ul className={`steps w-full ${steps ? 'steps-vertical lg:steps-horizontal' : ''}`}>
        {(steps || Array.from({ length: totalSteps }, (_, i) => ({ label: `Step ${i + 1}` }))).map((step: { label: string; description?: string }, index) => (
          <li 
            key={index}
          ...
          >
            <div className="text-left">
              ...
              {'description' in step && step.description && (
                <div className="text-sm text-base-content/60">{step.description as string}</div>
              )}
            </div>
          </li>
        ))}
