...
import { DEFAULT_MEM4AI_CONFIG, Mem4aiConfig } from './mem4aiConfig';
...
export class Mem4aiProvider {
  ...
  private getEmbeddingProviderId(): string | undefined {
    return (
      this.config.embeddingProviderId?.trim() ||
  ...
  async addMemory(content: string, metadata?: Record<string, unknown>): Promise<MemoryEntry> {
    ...
    try {
      const response = (await this.makeRequest('/memories', 'POST', body)) as Record<
        string,
        unknown
      >;
  ...
  async searchMemories(query: string, limit?: number): Promise<SearchResult[]> {
    ...
    try {
      ...
      const results: SearchResult[] = (Array.isArray(response.results) ? response.results : []).map((result: Record<string, unknown>) => ({
  ...
  async getMemories(limit?: number): Promise<MemoryEntry[]> {
    ...
    try {
      ...
      const memories: MemoryEntry[] = (Array.isArray(response.memories) ? response.memories : []).map((mem: Record<string, unknown>) => ({
        id: mem.id as string,
  ...
  async updateMemory(
    ...
    try {
      const response = (await this.makeRequest(`/memories/${id}`, 'PUT', body)) as Record<
        string,
        unknown
      >;
