...
export class Mem4aiProvider {
  ...
  async searchMemories(query: string, limit?: number): Promise<SearchResult[]> {
    ...
    try {
      ...
      const results: SearchResult[] = resultsList.map((result: Record<string, unknown>) => ({
  ...
  async getMemories(limit?: number): Promise<MemoryEntry[]> {
    ...
    try {
      ...
      const memoriesList = Array.isArray(response.memories) ? response.memories : [];
      const memories: MemoryEntry[] = memoriesList.map((mem: Record<string, unknown>) => ({
        id: mem.id as string,
    ...
    try {
      ...
      return {
        ...
        timestamp: response.updated_at
          ? new Date(response.updated_at as string).getTime()
          : Date.now(),
        tags: (response.tags as string[]) || [],
      };
    } catch (error) {
