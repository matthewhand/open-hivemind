...
export class SlackService extends EventEmitter implements IMessengerService {
   ...
   *
   * @returns {SlackService} The singleton instance
   * @throws {ConfigurationError} If instance creation fails
   * @example
   * ```typescript
   * const slackService = SlackService.getInstance();
   * await slackService.sendMessageToChannel('C123456', 'Hello!');
   * ```
   */
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      const startupGreetingService = container.resolve(require('@src/services/StartupGreetingService').default);
      startupGreetingService.emit('service-ready', this);
    } catch (error) {
      debug('Failed to emit service-ready, could not resolve StartupGreetingService:', error);
    }
  }
   ...
   * @param {string} [senderName] - Optional sender name override
   * @param {string} [threadId] - Optional thread ID to reply in
   ...
   * @throws {Error} If message sending fails after all retries
   * @example
   * ```typescript
   * const ts = await slackService.sendMessageToChannel(
   *   'C123456',
   *   'Hello, world!',
   *   'MyBot'
   * );
