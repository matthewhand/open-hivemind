...
describe('MemVaultProvider', () => {
  ...
  describe('constructor', () => {
    it('should throw if host is missing', () => {
      expect(() => new MemVaultProvider({ ...validConfig, host: '' })).toThrow(
        'MemVault PostgreSQL host is required'
      );
    });

    it('should throw if database is missing', () => {
      expect(() => new MemVaultProvider({ ...validConfig, database: '' })).toThrow(
        'MemVault PostgreSQL database is required'
      );
    });

    it('should throw if user is missing', () => {
      expect(() => new MemVaultProvider({ ...validConfig, user: '' })).toThrow(
        'MemVault PostgreSQL user is required'
      );
    });
