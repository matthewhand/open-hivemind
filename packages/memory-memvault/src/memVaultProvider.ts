...
import { Pool } from 'pg';
import { DEFAULT_MEMVAULT_CONFIG, MemVaultConfig } from './memVaultConfig';
...
export class MemVaultProvider {
  ...
  private getLlmProfileKey(): string | undefined {
    return (
      this.config.llmProfileKey?.trim() || this.config.embeddingProfileKey?.trim() || undefined
    );
  }
  ...
  private async generateEmbedding(content: string): Promise<number[]> {
    ...
    if (!this.config.embeddingEndpoint) {
      throw new Error(
        'LLM profile key + embedding resolver or embedding endpoint is required for vector generation'
      );
    }
