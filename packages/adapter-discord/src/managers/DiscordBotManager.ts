...
export class DiscordBotManager {
  private bots: Bot[] = [];
  /**
   * Dual-key cache storing O(1) references to bots by both their `botUserName` and `config.name`.
   * Note: We assume these names are static for the lifecycle of the bot connection. If a bot's name
   * is changed dynamically while connected, this cache would hold stale keys and need a manual rebuild.
   */
