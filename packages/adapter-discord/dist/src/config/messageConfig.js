...
const responseProfiles_1 = require("./responseProfiles");
const debug = (0, debug_1.default)('app:messageConfig');
// Utility: safe JSON parse with strict error throwing
function strictParseJSON(input) {
    try {
        ...
        throw new Error(`Invalid JSON: ${(e === null || e === void 0 ? void 0 : e.message) || String(e)}`);
    }
}
...
const responseProfileKeySet = new Set(responseProfiles_1.RESPONSE_PROFILE_OVERRIDE_KEYS);
function coerceResponseProfileValue(key, value) {
    if (!responseProfileKeySet.has(key)) {
        return undefined;
    }
    const expectedType = responseProfiles_1.RESPONSE_PROFILE_KEY_TYPES[key];
...
function normalizeChannelMaps(bonuses, priorities, knownChannels) {
    const outB = {};
    const outP = {};
...
const configPath = path_1.default.join(configDir, 'providers/message.json');
try {
    ...
    debug('Error loading config: %s', (error === null || error === void 0 ? void 0 : error.message) || String(error));
}
...
if (process.env.ALLOW_CONSOLE) {
    console.log('pre-normalize get(CHANNEL_BONUSES)=', messageConfig.get('CHANNEL_BONUSES'));
    console.log('pre-normalize get(CHANNEL_PRIORITIES)=', messageConfig.get('CHANNEL_PRIORITIES'));
    const propsPre = (_b = (_a = messageConfig).getProperties) === null || _b === void 0 ? void 0 : _b.call(_a);
    console.log('pre-normalize props keys=', propsPre ? Object.keys(propsPre) : 'no-props');
}
const normalized = normalizeChannelMaps(messageConfig.get('CHANNEL_BONUSES'), messageConfig.get('CHANNEL_PRIORITIES'), undefined);
...
if (process.env.ALLOW_CONSOLE) {
    console.log('post-normalize get(CHANNEL_BONUSES)=', messageConfig.get('CHANNEL_BONUSES'));
    console.log('post-normalize get(CHANNEL_PRIORITIES)=', messageConfig.get('CHANNEL_PRIORITIES'));
}
...
const _origGet = (_c = messageConfig.get) === null || _c === void 0 ? void 0 : _c.bind(messageConfig);
const _getProps = (_d = messageConfig.getProperties) === null || _d === void 0 ? void 0 : _d.bind(messageConfig);
if (_origGet) {
    messageConfig.get = (key) => {
