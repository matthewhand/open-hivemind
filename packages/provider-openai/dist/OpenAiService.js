...
const redactSensitiveInfo_1 = require("@common/redactSensitiveInfo");
...
if (!openaiConfig_1.default || typeof openaiConfig_1.default.get !== 'function') {
    throw new errorClasses_1.ConfigurationError('Invalid OpenAI configuration: expected an object with a get method.', 'OPENAI_CONFIG_VALIDATION_ERROR');
}
// Guard: Validate llmConfig object
if (!llmConfig_1.default || typeof llmConfig_1.default.get !== 'function') {
    throw new errorClasses_1.ConfigurationError('Invalid LLM configuration: expected an object with a get method.', 'LLM_CONFIG_VALIDATION_ERROR');
}
class OpenAiService {
    /**
    ...
    constructor() {
        ...
        const timeoutValue = Number(openaiConfig_1.default.get('OPENAI_TIMEOUT') || 300000);
        ...
        this.parallelExecution = llmConfig_1.default.get('LLM_PARALLEL_EXECUTION') === true; // Explicitly check for boolean
     ...
     */
    async generateChatCompletion(message, historyMessages, systemMessageContent = String(openaiConfig_1.default.get('OPENAI_SYSTEM_PROMPT') || ''), maxTokens = Number(openaiConfig_1.default.get('OPENAI_RESPONSE_MAX_TOKENS') || 150), temperature = Number(openaiConfig_1.default.get('OPENAI_TEMPERATURE') || 0.7)) {
        var _a;
        ...
        try {
            const response = await this.retryWithBackoff(async () => {
                return await this.openai.chat.completions.create({
                    model: openaiConfig_1.default.get('OPENAI_MODEL') || 'gpt-4o',
                    messages: chatParams,
            ...
            debug('[DEBUG] OpenAI API response received:', (0, redactSensitiveInfo_1.redactSensitiveInfo)('response', JSON.stringify(response)));
            const choice = response.choices && response.choices[0];
            debug('[DEBUG] Raw first choice:', JSON.stringify(choice));
            let content = ((_a = choice === null || choice === void 0 ? void 0 : choice.message) === null || _a === void 0 ? void 0 : _a.content) || '';
            debug('[DEBUG] Extracted content from choices:', content);
            if (!content && response.full_response && Array.isArray(response.full_response.messages)) {
                const messages = response.full_response.messages;
            ...
            let activeAgentName;
            if (response.full_response && response.full_response.agent && response.full_response.agent.name) {
                activeAgentName = response.full_response.agent.name;
            }
            return {
                text: content,
            ...
            debug('[DEBUG] Error generating chat completion:', { message, historyMessages, error: errors_1.ErrorUtils.getMessage(hivemindError) });
            throw hivemindError;
        }
    }
    /**
     ...
     */
    async generateChatResponse(message, historyMessages, systemMessageContent = String(openaiConfig_1.default.get('OPENAI_SYSTEM_PROMPT') || ''), maxTokens = Number(openaiConfig_1.default.get('OPENAI_RESPONSE_MAX_TOKENS') || 150), temperature = Number(openaiConfig_1.default.get('OPENAI_TEMPERATURE') || 0.7)) {
    ...
    async listModels() {
        ...
        try {
            const models = await (0, listModels_1.listModels)(this.openai);
            debug('[DEBUG] Models retrieved:', models);
            return models;
        }
        catch (error) {
            const hivemindError = errors_1.ErrorUtils.toHivemindError(error);
            debug('[DEBUG] Error listing models:', errors_1.ErrorUtils.getMessage(hivemindError));
            throw hivemindError;
        }
    }
    /**
     ...
     */
    classifyError(error) {
        const statusCode = errors_1.ErrorUtils.getStatusCode(error);
        const errorCode = errors_1.ErrorUtils.getCode(error);
        if (statusCode === 429) {
            return 'rate-limit';
        }
     ...
     */
    redactApiKeyForLogging(key) {
        debug('[DEBUG] redactApiKeyForLogging called');
        if (debug.enabled) {
            const redacted = (0, redactSensitiveInfo_1.redactSensitiveInfo)('OPENAI_API_KEY', key);
