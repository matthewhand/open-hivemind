...
export async function startTyping(client: Client, channelId: string): Promise<void> {
    try {
        debug('[DiscordManager] Fetching channel ID: ' + channelId);
        const channel = await client.channels.fetch(channelId);
        debug('[DiscordManager] Fetched channel: ' + (channel ? channel.id : 'null'));
        if (!channel) {
            debug('[DiscordManager] Channel with ID: ' + channelId + ' not found.');
            return;
        }
        debug('[DiscordManager] Channel type: ' + channel.type);
        if (channel instanceof TextChannel || channel instanceof NewsChannel) {
            ...
            debug('[DiscordManager] Started typing in channel ID: ' + channelId);
        } else {
            debug('[DiscordManager] Channel ID: ' + channelId + ' does not support typing.');
        }
    } catch (error: any) {
