...
export const ToolUsageGuardSchema = z.object({
  body: z.object({
    ...
    guardType: z.enum(['owner_only', 'user_list', 'role_based'], { message: 'guardType must be one of: owner_only, user_list, role_based' }),
...
export const LlmProviderSchema = z.object({
  body: z.object({
    ...
    config: z.record(z.any()).refine(val => Object.keys(val).length > 0, { message: 'Name, type, and config are required' }),
  }).passthrough(),
});
