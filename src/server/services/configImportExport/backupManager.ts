...
export class BackupManager {
  ...
  async createBackup(
    ...
    try {
      ...
      if (result.success && result.filePath) {
        ...
        const backupPath = this.getSafeBackupPath(
          name,
          new Date(backupTimestamp),
          ...
          if (allBackups.length > maxBackups) {
            ...
            for (const oldBackup of backupsToDelete) {
              if (enableColdStorage) {
                ...
                const oldBackupPath = this.getSafeBackupPath(
                  oldBackup.name,
                  oldBackup.createdAt,
                  oldBackup.encrypted
                );
   ...
   */
  getSafeBackupPath(name: string, createdAt: Date, encrypted: boolean = false): string {
    const sanitizedName = PathSecurityUtils.sanitizeFilename(name);
    const ext = encrypted ? '.json.enc.gz' : '.json.gz';
    const backupFileName = `backup-${sanitizedName}-${createdAt.getTime()}${ext}`;
