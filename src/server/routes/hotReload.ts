...
router.post('/api/config/hot-reload', validateRequest(HotReloadChangeSchema), async (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Hot reload failed'));
  }
});
...
router.get('/api/config/hot-reload/history', (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to get change history'));
  }
});
...
router.get('/api/config/hot-reload/rollbacks', (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to get available rollbacks'));
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (success) {
        ...
        return res.status(HTTP_STATUS.NOT_FOUND).json(ApiResponse.error('Rollback snapshot not found or rollback failed'));
      }
    } catch (error) {
      debug('Hot reload rollback API error:', error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Rollback failed'));
    }
  }
);
...
router.get('/api/config/hot-reload/status', (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to get hot reload status'));
  }
});
