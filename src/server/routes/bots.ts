...
router.get('/', async (req, res) => {
  try {
    ...
    logger.error('Failed to retrieve bots', new Error(error.message));
...
router.get('/:id', validateRequest(BotIdParamSchema), async (req, res) => {
  try {
    ...
    logger.error(`Failed to retrieve bot ${req.params.id}`, new Error(error.message));
...
router.get('/:id/activity', validateRequest(BotActivityQuerySchema), async (req, res) => {
  try {
    ...
    logger.error(`Failed to retrieve bot activity for ${req.params.id}`, new Error(error.message));
