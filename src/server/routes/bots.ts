...
router.get('/', async (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to retrieve bots', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
...
router.put('/reorder', validateRequest(ReorderSchema), async (req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to reorder bots', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
...
router.get('/export', async (_req, res) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to export bots', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
  ...
  try {
    ...
    if (!Array.isArray(incoming) || incoming.length === 0) {
      return res
        ...
        .json(ApiResponse.error('Request body must contain a non-empty "bots" array', String(HTTP_STATUS.BAD_REQUEST)));
    }
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to import bots', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
...
router.get('/:id', validateRequest(BotIdParamSchema), async (req, res) => {
  try {
    ...
    if (!bot) {
      return res.status(HTTP_STATUS.NOT_FOUND).json(ApiResponse.error('Bot not found', String(HTTP_STATUS.NOT_FOUND)));
    }
    return res.json(ApiResponse.success({ bot }));
  } catch (error: unknown) {
    logger.error(
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to retrieve bot', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
  ...
  try {
    ...
    if (existingBot) {
      return res
        ...
        .json(ApiResponse.success({ bot: existingBot }));
    }

    const bot = await manager.createBot(request);
    return res.status(HTTP_STATUS.CREATED).json(ApiResponse.success({ bot }));
  } catch (error: any) {
    return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error(error.message, String(HTTP_STATUS.BAD_REQUEST)));
  }
});
...
router.put('/:id', validateRequest(UpdateBotSchema), async (req, res) => {
  try {
    ...
    return res.status(status).json(ApiResponse.error(error.message, String(status)));
  }
});
...
router.post('/:id/clone', validateRequest(CloneBotSchema), async (req, res) => {
  try {
    ...
    return res
      ...
      .json(ApiResponse.success({ bot: newBot }));
  } catch (error: any) {
    const status = error.message.includes(ERROR_CODES.NOT_FOUND)
      ? HTTP_STATUS.NOT_FOUND
      : HTTP_STATUS.BAD_REQUEST;
...
router.get('/:id/history', validateRequest(BotHistoryQuerySchema), async (req, res) => {
  try {
    ...
    return res.json(ApiResponse.success({ data: { history } }));
  } catch (error: any) {
    const status = error.message.includes(ERROR_CODES.NOT_FOUND)
      ? HTTP_STATUS.NOT_FOUND
      : HTTP_STATUS.BAD_REQUEST;
...
router.get('/:id/activity', validateRequest(BotActivityQuerySchema), async (req, res) => {
  try {
    ...
    return res.json(ApiResponse.success({ data: { activity } }));
  } catch (error: unknown) {
    logger.error(
    ...
    return res
      ...
      .json(ApiResponse.error('Failed to retrieve bot activity', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
  ...
  try {
    ...
    if (!bot) {
      return res
        ...
        .json(ApiResponse.error('Bot not found', String(HTTP_STATUS.NOT_FOUND), { code: ERROR_CODES.NOT_FOUND }));
    }
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to export bot', String(HTTP_STATUS.INTERNAL_SERVER_ERROR)));
  }
});
