...
const handleValidationErrors = (req: Request, res: Response, next: any) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error('Validation failed', undefined, {
      errors: errors.array()
    }));
  }
  return next();
...
const handleUploadError = (error: any, req: Request, res: Response, next: any) => {
  if (error instanceof multer.MulterError) {
    if (error.code === 'LIMIT_FILE_SIZE') {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error('File too large. Maximum size is 50MB.'));
    }
    return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error(`File upload error: ${error.message}`));
  } else if (error) {
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (result.success) {
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error('Export failed'));
      }
    } catch (error) {
      console.error('Error exporting configurations:', error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to export configurations'));
    }
  }
);
...
router.post(
  ...
  async (req: AuthMulterRequest, res: Response) => {
    try {
      ...
      return res.json(result.success ? ApiResponse.success(result) : ApiResponse.error(
        result.success ? 'Configurations imported successfully' : 'Import failed',
        undefined,
        {
          data: result
        }
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to import configurations'));
    }
  }
);
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (result.success) {
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error('Backup creation failed'));
      }
    } catch (error) {
      console.error('Error creating backup:', error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to create backup'));
    }
  }
);
...
router.get('/backups', requireAdmin, async (req: AuthMiddlewareRequest, res: Response) => {
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to list backups'));
  }
});
...
router.post(
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (!backupPath) {
        return res.status(HTTP_STATUS.NOT_FOUND).json(ApiResponse.error('Backup not found or invalid'));
      }
      ...
      return res.json(result.success ? ApiResponse.success(result) : ApiResponse.error(
        result.success ? 'Backup restored successfully' : 'Backup restoration failed',
        undefined,
        {
          data: result
        }
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to restore from backup'));
    }
  }
);
...
router.delete(
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to delete backup'));
    }
  }
);
...
router.get(
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to download backup'));
    }
  }
);
...
router.post(
  ...
  async (req: AuthMulterRequest, res: Response) => {
    try {
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(ApiResponse.error('Failed to validate file'));
    }
  }
);
