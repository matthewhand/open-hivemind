  ...
  try {
    ...
    if (!message) {
      res
        ...
        .json(ApiResponse.error('An error occurred', undefined, { error: 'message is required' }));
      return;
    }
    ...
    if (!botName) {
      res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json(ApiResponse.error('An error occurred', undefined, { error: 'botName is required' }));
      return;
    }
