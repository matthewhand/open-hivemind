...
const handleValidationErrors = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!errors.isEmpty()) {
    ...
    return res.status(HTTP_STATUS.BAD_REQUEST).json(ApiResponse.error(message, code, details));
  }
  return next();
};
...
router.get('/api/validation', async (req: AuthMiddlewareRequest, res: Response) => {
  try {
    ...
    return res
      ...
      .json(ApiResponse.error(errorMessage, 'VALIDATION_ERROR'));
  }
});
    ...
    try {
      ...
      if (!config) {
        return res
          ...
          .json(ApiResponse.error('Configuration data required'));
      }

      if (typeof config !== 'object' || Array.isArray(config)) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json(ApiResponse.error('Configuration data must be an object'));
      }
...
router.get('/api/validation/schema', (_req: AuthMiddlewareRequest, res: Response) => {
  ...
  try {
    return res.json(ApiResponse.success(schema));
  } catch (error: unknown) {
    const hivemindError = ErrorUtils.toHivemindError(
      error,
    ...
    try {
      ...
      if (!rule) {
        return res
          ...
          .json(ApiResponse.error('Validation rule not found'));
      }
      ...
      return res
        .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
        .json(ApiResponse.error(hivemindError.message, hivemindError.code));
    }
  }
);
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (existingRule) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json(ApiResponse.error('Validation rule with this ID already exists'));
      }
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (!profile) {
        return res
          .status(HTTP_STATUS.NOT_FOUND)
          .json(ApiResponse.error('Validation profile not found'));
      }
  ...
  async (req: AuthMiddlewareRequest, res: Response) => {
    try {
      ...
      if (existingProfile) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json(ApiResponse.error('Validation profile with this ID already exists'));
      }
      ...
      if (invalidRuleIds.length > 0) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json(ApiResponse.error(`Invalid rule IDs: ${invalidRuleIds.join(', ')}`));
      }
