...
const router = Router();
const dashboardLogger = Logger.withContext('routes:dashboard');
...
router.get('/ai/stats', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    dashboardLogger.error('AI stats API error:', error);
...
router.get('/ai/segments', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    dashboardLogger.error('AI segments API error:', error);
...
router.get('/ai/patterns', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    dashboardLogger.error('AI patterns API error:', error);
    res
...
router.get('/ai/recommendations', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    dashboardLogger.error('AI recommendations API error:', error);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      dashboardLogger.error('Error storing AI feedback:', error);
...
router.get('/status', authenticate, requireAdmin, (req, res) => {
  try {
    ...
    try {
      ...
      dashboardLogger.warn('Failed to load bots for status:', e);
      bots = [];
    }
    ...
    dashboardLogger.error('Status API error:', error);
...
router.get('/activity', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    dashboardLogger.error('Activity API error:', error);
    res
      .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      dashboardLogger.error('Acknowledge alert error:', error);
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      dashboardLogger.error('Resolve alert error:', error);
