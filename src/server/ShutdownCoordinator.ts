...
export class ShutdownCoordinator {
  ...
  private async phaseStopBackground(): Promise<void> {
    ...
    try {
      ...
      if (idleManager && typeof idleManager.shutdown === 'function') {
        idleManager.shutdown();
        debug('IdleResponseManager shut down');
      }
    } catch (error) {
      debug('Error shutting down IdleResponseManager:', error);
    }

    // Stop BotMetricsService
    try {
      const { BotMetricsService } = require('@server/services/BotMetricsService');
      const botMetrics = BotMetricsService.getInstance();
      if (botMetrics && typeof botMetrics.stop === 'function') {
        botMetrics.stop();
        debug('BotMetricsService stopped');
      }
    } catch (error) {
      debug('Error stopping BotMetricsService:', error);
    }

    // Stop RealTimeValidationService
    try {
      const { RealTimeValidationService } = require('@server/services/RealTimeValidationService');
      const validationService = RealTimeValidationService.getInstance();
      if (validationService && typeof validationService.shutdown === 'function') {
        validationService.shutdown();
        debug('RealTimeValidationService shut down');
      }
    } catch (error) {
      debug('Error shutting down RealTimeValidationService:', error);
    }

    // Stop ApiMonitorService
    try {
      const { default: ApiMonitorService } = require('@services/ApiMonitorService');
      const apiMonitor = ApiMonitorService.getInstance();
      if (apiMonitor && typeof apiMonitor.shutdown === 'function') {
        apiMonitor.shutdown();
        debug('ApiMonitorService shut down');
      }
    } catch (error) {
      debug('Error shutting down ApiMonitorService:', error);
    }

    // Stop AnomalyDetectionService
    try {
      const { default: AnomalyDetectionService } = require('@services/AnomalyDetectionService');
      const anomalyService = AnomalyDetectionService.getInstance();
      if (anomalyService && typeof anomalyService.shutdown === 'function') {
        anomalyService.shutdown();
        debug('AnomalyDetectionService shut down');
      }
    } catch (error) {
      debug('Error shutting down AnomalyDetectionService:', error);
    }
    ...
    try {
      ...
      if (metricsCollector && typeof metricsCollector.shutdown === 'function') {
        ...
        debug('MetricsCollector shut down');
      }
    } catch (error) {
      debug('Error shutting down MetricsCollector:', error);
    }

    // Stop IntegrationAnomalyDetector
    try {
      const { IntegrationAnomalyDetector } = require('@monitoring/IntegrationAnomalyDetector');
      const integrationDetector = IntegrationAnomalyDetector.getInstance();
      if (integrationDetector && typeof integrationDetector.shutdown === 'function') {
        integrationDetector.shutdown();
        debug('IntegrationAnomalyDetector shut down');
      }
    } catch (error) {
      debug('Error shutting down IntegrationAnomalyDetector:', error);
    }

    // Stop ProviderMetricsCollector
    try {
      const { ProviderMetricsCollector } = require('@monitoring/ProviderMetricsCollector');
      const providerMetrics = ProviderMetricsCollector.getInstance();
      if (providerMetrics && typeof providerMetrics.shutdown === 'function') {
        providerMetrics.shutdown();
        debug('ProviderMetricsCollector shut down');
      }
    } catch (error) {
      debug('Error shutting down ProviderMetricsCollector:', error);
    }
    ...
    try {
      ...
      debug('Error shutting down MCPProviderManager:', error);
    }

    // Stop UsageTrackerService
    try {
      const { UsageTrackerService } = require('@server/services/UsageTrackerService');
      const usageTracker = UsageTrackerService.getInstance();
      if (usageTracker && typeof usageTracker.shutdown === 'function') {
        await usageTracker.shutdown();
        debug('UsageTrackerService shut down');
      }
    } catch (error) {
      debug('Error shutting down UsageTrackerService:', error);
    }

    // Stop ToolPreferencesService
    try {
      const { ToolPreferencesService } = require('@server/services/ToolPreferencesService');
      const toolPrefs = ToolPreferencesService.getInstance();
      if (toolPrefs && typeof toolPrefs.shutdown === 'function') {
        await toolPrefs.shutdown();
        debug('ToolPreferencesService shut down');
      }
    } catch (error) {
    ...
    try {
      ...
      if (profiler && typeof profiler.destroy === 'function') {
        profiler.destroy();
        debug('PerformanceProfiler destroyed');
      }
    } catch (error) {
      debug('Error destroying PerformanceProfiler:', error);
    }
    ...
    try {
      ...
      if (timerRegistry && typeof timerRegistry.clearAll === 'function') {
        timerRegistry.clearAll();
        debug('TimerRegistry cleared');
      }
    } catch (error) {
      debug('Error clearing TimerRegistry:', error);
    }

    // Stop rate limiter cleanup
    try {
      const { shutdownRateLimiter } = require('@middleware/rateLimiter');
      if (typeof shutdownRateLimiter === 'function') {
        shutdownRateLimiter();
        debug('Rate limiter shut down');
      }
    } catch (error) {
      debug('Error shutting down rate limiter:', error);
    }

    // Stop CSRF token cleanup
    try {
      const { stopTokenCleanup } = require('@server/middleware/csrf');
      if (typeof stopTokenCleanup === 'function') {
        stopTokenCleanup();
        debug('CSRF token cleanup stopped');
      }
    } catch (error) {
      debug('Error stopping CSRF token cleanup:', error);
    }
