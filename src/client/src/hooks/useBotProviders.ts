...
import type { LLMProvider, MessageProvider } from '../types/bot';
...
const useBotProviders = () => {
  ...
  const createMessageProvider = useCallback(
    (_botId: string, type: string, name: string, config: Record<string, any>): MessageProvider => {
  ...
  const createLLMProvider = useCallback(
    (_botId: string, type: string, name: string, config: Record<string, any>): LLMProvider => {
      const schema = getProviderSchema(type);
  ...
  const addMessageProvider = useCallback(
    (botId: string, type: string, name: string, config: Record<string, any>) => {
  ...
  const updateProvider = useCallback(
    (botId: string, providerId: string, updates: Partial<MessageProvider | LLMProvider>) => {
      updateBot(botId, (prevBot) => {
  ...
  const removeProvider = useCallback(
    (botId: string, providerId: string) => {
      updateBot(botId, (prevBot) => ({
        ...prevBot,
  ...
  const testProvider = useCallback(
    async (botId: string, providerId: string): Promise<boolean> => {
      try {
