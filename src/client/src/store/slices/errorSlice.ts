...
export const selectErrorStats = (state: { error: ErrorState }) => {
  ...
  // ⚡ Bolt Optimization: Calculate error stats in a single O(N) pass instead of 4 separate O(N) passes
  let unresolved = 0;
  let resolved = 0;
  ...
  return {
    total,
    byType: byType as Record<ErrorDetails['type'], number>,
    bySeverity: bySeverity as Record<ErrorDetails['severity'], number>,
    unresolved,
    resolved,
