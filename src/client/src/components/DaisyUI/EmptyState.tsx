...
const variantStyles = {
  primary: {
    ...
    shadow: 'shadow-primary/25 hover:shadow-primary/40',
  },
  secondary: {
  ...
  warning: {
    ...
    shadow: 'shadow-warning/25 hover:shadow-warning/40',
  },
  error: {
    ...
    shadow: 'shadow-error/25 hover:shadow-error/40',
  },
};
...
const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  // Map variant to button variant, defaulting to primary if not supported by Button types safely
  // Since Button types are strict, we'll cast or map.
  // For now, let's keep the button as primary/secondary/accent if possible, or primary for others.
  let buttonVariant: 'primary' | 'secondary' | 'accent' = 'primary';
  if (variant === 'secondary') buttonVariant = 'secondary';
  if (variant === 'accent') buttonVariant = 'accent';
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        {actionLabel && onAction && (
          <Button
            ...
            onClick={onAction}
            className={`gap-2 shadow-lg ${styles.shadow} transition-shadow`}
