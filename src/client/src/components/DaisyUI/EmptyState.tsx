...
const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  // Map EmptyState variant to Button variant
  // Only pass valid Button variants, otherwise default to 'primary'
  const buttonVariant = ['primary', 'secondary', 'accent', 'ghost', 'link'].includes(variant)
    ? variant as 'primary' | 'secondary' | 'accent' | 'ghost' | 'link'
    : 'primary';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-30">
        <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-base-100/10 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-40 h-40 bg-base-100/10 rounded-full blur-3xl" />
      </div>
