  ...
  useEffect(() => {
    const fetchSystemData = async () => {
      try {
        ...
        // We use a try-catch block here to fallback to mocks if the API fails or is not implemented fully
        try {
            const [healthData, endpointsStatus] = await Promise.all([
            ...
            if (healthData) {
                ...
                const newMetrics: SystemMetrics = {
                  cpu: {
                    usage: Math.random() * 100, // Mock: API doesn't provide % directly usually
                    cores: 8, // Mock
                    temperature: 45 + Math.random() * 10, // Mock
                  },
                  memory: {
                  ...
                  disk: {
                    used: 250 * 1024 * 1024 * 1024, // Mock 250GB
                    total: 500 * 1024 * 1024 * 1024, // Mock 500GB
                    usage: 50, // Mock
                  },
                  network: {
                    latency: 24, // Mock
            ...
            if (endpointsStatus && endpointsStatus.endpoints) {
                const checks: HealthCheck[] = endpointsStatus.endpoints.map(ep => ({
                    id: ep.id,
                    name: ep.name,
                ...
                // If no endpoints are configured, add a dummy one so the UI isn't empty
                if (checks.length === 0) {
                    checks.push({
                        id: 'db',
                        name: 'Database',
                ...
                throw new Error("No endpoints data");
            }

        } catch (innerErr) {
            // Fallback to pure mock data if API fails
