      ...
      <div key={key} className="form-control">
        ...
        {(type === 'text' || type === 'number' || type === 'password') && (
          <div className="join w-full">
            ...
            />
            {isLocked && <button aria-label="Value is locked from environment" className="btn btn-sm btn-square join-item btn-disabled"><LockClosedIcon className="w-4 h-4" /></button>}
          </div>
        )}
        {type === 'select' && (
          <Select
