...
const MonitoringDashboard: React.FC<MonitoringDashboardProps> = ({
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // In a real app, statusData might come from getStatus() (systemData.bots)
      // systemData.bots structure: { name, provider, llmProvider, status, ... }

      const statusMap = new Map(systemData.bots.map(b => [b.name, b]));

      const botsWithStatus = configData.bots.map((bot: Bot) => {
        const status = statusMap.get(bot.name);
        return {
          ...bot,
          id: bot.name,
          statusData: status ? {
            ...
            messageCount: status.messageCount ?? Math.floor(Math.random() * 100),
            errorCount: status.errorCount ?? 0,
            responseTime: (status as any).responseTime ?? Math.floor(Math.random() * 500) + 100,
            uptime: (status as any).uptime ?? Math.floor(Math.random() * 86400),
            lastActivity: (status as any).lastActivity ?? new Date().toISOString(),
          } : {
            status: 'unknown',
            connected: false,
            messageCount: 0,
  ...
  const getHealthColor = (status: string) => {
    ...
    default: return 'info'; // Changed from ghost to info for StatsCards
    }
  };
  ...
  return (
    <div className="flex-1 space-y-6">
      ...
      {/* Tab Navigation */}
      <div className="bg-base-200 border-b border-base-300 rounded-box">
