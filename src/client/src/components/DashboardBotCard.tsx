        ...
        <div className="card-body p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
               <Tooltip content="Run Performance Benchmark" position="top" className="opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                 <button
                   type="button"
                   onClick={() => setIsBenchmarkOpen(true)}
                   className="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100 transition-opacity text-warning"
                   aria-label={`Run performance benchmark for ${bot.name}`}
                 >
                    <Trophy className="w-4 h-4" />
                 </button>
               </Tooltip>
               <Tooltip content="Version History" position="top" className="opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                 <button
                   type="button"
                   onClick={() => setIsHistoryOpen(true)}
                   className="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100 transition-opacity text-secondary"
                   aria-label={`View version history for ${bot.name}`}
                 >
                    <History className="w-4 h-4" />
                 </button>
               </Tooltip>
               <Tooltip content="AI Performance Insights" position="top" className="opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                 <button
                   type="button"
                   onClick={() => setIsInsightsOpen(true)}
                   className="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100 transition-opacity text-primary"
                   aria-label={`View AI performance insights for ${bot.name}`}
                 >
                    <Sparkles className="w-4 h-4" />
                 </button>
               </Tooltip>
               <Tooltip content="Run Diagnostic" position="top" className="opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                 <button
                   type="button"
                   onClick={() => setIsDiagnosticOpen(true)}
                   className="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100 transition-opacity"
