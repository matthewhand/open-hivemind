          ...
          <div className="bg-base-200/20 p-4 rounded-xl max-h-[60vh] overflow-y-auto">
             <ul className="timeline timeline-vertical">
                {versions.map((v, index) => {
                  ...
                  const isLastItem = index === versions.length - 1;
                  return (
                    <li key={v.version}>
                      {index > 0 && <hr />}
                      <div className="timeline-start text-[10px] opacity-40 font-mono text-right w-16 leading-tight">{new Date(v.createdAt).toLocaleDateString()}<br/>{new Date(v.createdAt).toLocaleTimeString()}</div>
                      <div className="timeline-middle">
                        <div className={`p-1.5 rounded-full ${isLatest ? 'bg-primary text-primary-content ring-4 ring-primary/20' : 'bg-base-300 text-base-content/50'}`}>{isLatest ? <Check className="w-3 h-3" /> : <Clock className="w-3 h-3" />}</div>
                      </div>
