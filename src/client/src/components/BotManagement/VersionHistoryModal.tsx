             ...
             <ul className="timeline timeline-vertical">
                {versions.map((v, index) => {
                  ...
                  return (
                    <li key={v.version}>
                      {index > 0 && <hr />}
                      <div className="timeline-start text-[10px] opacity-40 font-mono text-right w-16 leading-tight">
                        {new Date(v.createdAt).toLocaleDateString()}<br />{new Date(v.createdAt).toLocaleTimeString()}
                      </div>
                      <div className="timeline-middle">
                        <div className={`p-1.5 rounded-full ${isLatest ? 'bg-primary text-primary-content ring-4 ring-primary/20' : 'bg-base-300 text-base-content/50'}`}>
                          {isLatest ? <Check className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                        </div>
                      </div>
                      <div className="timeline-end w-full mb-4">
                        <div className={`p-4 rounded-xl border transition-all ${isLatest ? 'bg-base-100 border-primary/30 shadow-md' : 'bg-base-100/50 border-base-300 hover:border-base-content/20 hover:bg-base-100'}`}>
