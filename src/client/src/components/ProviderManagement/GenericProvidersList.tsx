...
const GenericProvidersList: React.FC<GenericProvidersListProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex justify-center py-12" aria-busy="true"><LoadingSpinner size="lg" /></div>
      ) : profiles.length === 0 ? (
        <EmptyState
          icon={emptyStateIcon}
              ...
              <div className="card-body p-0">
                <div className="p-4 flex items-center justify-between cursor-pointer" onClick={() => toggleExpand(profile.key)}>
                  ...
                  <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" variant="ghost" onClick={() => onEditProfile(profile)} aria-label={`Edit ${profile.name}`}>
                      <EditIcon className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" className="text-error hover:bg-error/10" onClick={() => onDeleteProfile(profile.key)} aria-label={`Delete ${profile.name}`}>
                      <DeleteIcon className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => toggleExpand(profile.key)} aria-label={expandedProfile === profile.key ? `Collapse ${profile.name}` : `Expand ${profile.name}`}>
