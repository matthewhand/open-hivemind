      ...
      {formModal.isOpen && (
        <dialog className="modal modal-open">
          <div className="modal-box max-w-lg">
            <h3 className="font-bold text-lg mb-4">{formModal.isEdit ? 'Edit Tool Profile' : 'Create Tool Profile'}</h3>
            <div className="form-control w-full mb-3">
              <label className="label" htmlFor="tool-profile-name"><span className="label-text">Profile Name</span></label>
              <input id="tool-profile-name" type="text" className="input input-bordered w-full" placeholder="My Tool Provider" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} />
            </div>
            <div className="form-control w-full mb-3">
              <label className="label" htmlFor="tool-profile-provider"><span className="label-text">Provider Type</span></label>
              <select id="tool-profile-provider" className="select select-bordered w-full" value={selectedProvider} onChange={(e) => { setSelectedProvider(e.target.value); setFormData(prev => ({ ...prev, provider: e.target.value, config: {} })); }} disabled={formModal.isEdit}>
            ...
            {currentSchema?.fields?.map((field) => (
              <div key={field.name} className="form-control w-full mb-3">
                ...
                {field.type === 'select' ? (
                  <select id={`tool-field-${field.name}`} className="select select-bordered w-full" value={formData.config[field.name] ?? field.defaultValue ?? ''} onChange={(e) => setFormData(prev => ({ ...prev, config: { ...prev.config, [field.name]: e.target.value } }))}>
                  ...
                  <input id={`tool-field-${field.name}`} type={field.type === 'password' ? 'password' : 'text'} className="input input-bordered w-full" placeholder={field.placeholder || ''} value={formData.config[field.name] ?? ''} onChange={(e) => setFormData(prev => ({ ...prev, config: { ...prev.config, [field.name]: e.target.value } }))} />
                )}
