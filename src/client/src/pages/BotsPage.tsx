...
const BotsPage: React.FC = () => {
  ...
  const filteredBots = bots.filter(bot =>
    ...
    ((bot as any).messageProvider?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
        ...
        <div className="card-body p-0">
          {loading && bots.length === 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-base-content/60">No bots found matching "{searchQuery}"</p>
              <button className="btn btn-link btn-sm" onClick={() => setSearchQuery('')}>Clear Search</button>
            </div>
          ) : (
