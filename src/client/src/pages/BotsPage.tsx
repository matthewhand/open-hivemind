...
const BotsPage: React.FC = () => {
  ...
  const filteredBots = bots.filter(bot => {
    ...
    return (
      ...
      ((bot as any).messageProvider || '').toLowerCase().includes(query) ||
      ...
      <div className="card bg-base-100 border border-base-300">
        <div className="card-body p-0">
          {loading && bots.length === 0 ? (
            ...
            <div className="text-center py-12">
              <Search className="w-12 h-12 mx-auto text-base-content/20 mb-3" />
              <h3 className="text-lg font-medium text-base-content/60">No bots found</h3>
              <p className="text-base-content/50">No bots found matching "{searchQuery}"</p>
              <button className="btn btn-ghost mt-2" onClick={() => setSearchQuery('')}>Clear Search</button>
            </div>
          ) : (
