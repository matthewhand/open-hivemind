...
const MCPServersPage: React.FC = () => {
  ...
  const handleTestConnection = async () => {
    ...
    if (!selectedServer?.url) {
      setTestMessage({ type: 'error', message: 'Server URL is required' });
      return;
    }
    ...
    try {
      ...
      setTestMessage({ type: 'success', message: 'Connection successful!' });
    } catch (err) {
      setTestMessage({ type: 'error', message: err instanceof Error ? err.message : 'Connection failed' });
    } finally {
      setIsTesting(false);
    }
  };
    ...
    <div className="p-6">
      ...
      >
        <div className="space-y-4">
          {testMessage && (
            <Alert
              status={testMessage.type === 'success' ? 'success' : 'error'}
