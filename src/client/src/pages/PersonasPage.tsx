...
const PersonasPage: React.FC = () => {
  ...
  const openViewModal = (persona: Persona) => {
    ...
    setCloningPersonaId(null);
    setShowEditModal(true); // Reuse modal
  };
  ...
  return (
    <div className="space-y-6">
                ...
                {persona.assignedBotNames.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    ...
                    {persona.assignedBotNames.length > 3 && (
                      <Badge variant="ghost" size="small" className="text-xs opacity-70">
      ...
      <Modal
        ...
        onClose={() => { setShowCreateModal(false); setShowEditModal(false); setViewingPersona(null); }}
        ...
        <div className="space-y-4">
          <div className="form-control">
            ...
            <Input
              ...
              disabled={!!viewingPersona || (!!editingPersona && editingPersona.isBuiltIn)}
            />
          </div>
              ...
              {bots.length === 0 ? <div className="p-2 text-sm opacity-50">No bots available</div> :
                bots.map((bot: any) => {
                  ...
                  return (
                    <label
                      key={bot.id}
                      className={`cursor-pointer label justify-start gap-3 rounded-lg ${isEnvLocked || viewingPersona ? 'opacity-50 cursor-not-allowed' : 'hover:bg-base-300'}`}
          ...
          <div className="flex justify-end gap-2 mt-6">
            <Button variant="ghost" onClick={() => { setShowCreateModal(false); setShowEditModal(false); setViewingPersona(null); }}>
