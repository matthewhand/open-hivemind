...
const PersonasPage: React.FC = () => {
  ...
  const openViewModal = (persona: Persona) => {
    ...
    setEditingPersona(persona);
    setCloningPersonaId(null);
    setShowEditModal(true); // Reuse edit modal for view
  };
        ...
        <div className="space-y-4">
          <div className="form-control">
            ...
            <Input
              ...
              disabled={isViewOnly || (!!editingPersona && editingPersona.isBuiltIn)}
            />
          </div>
              ...
              {bots.length === 0 ? <div className="p-2 text-sm opacity-50">No bots available</div> :
                bots.map((bot: any) => {
                  ...
                  return (
                    <label
                      key={bot.id}
                      className={`cursor-pointer label justify-start gap-3 rounded-lg ${isEnvLocked || isViewOnly ? 'opacity-50 cursor-not-allowed' : 'hover:bg-base-300'}`}
