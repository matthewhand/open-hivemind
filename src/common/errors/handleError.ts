...
function isError(obj: any): obj is Error {
  return obj instanceof Error || (obj && typeof obj === 'object' && typeof obj.message === 'string');
}
...
/**
 * Handles errors by logging them and optionally sending a stable error message to a message channel.
...
export function handleError(error: unknown, messageChannel: any = null): void {
  ...
  if (!isError(error)) {
    ...
    if (isValidMessageChannel(messageChannel)) {
      ...
      try { messageChannel.send(errorMsg); } catch (sendError) { debug('Failed to send error message:', sendError); }
    }
    return;
  }
