...
class NetworkError extends BaseHivemindError {
  constructor(message, response, request, context) {
    super(
      ...
      'network',
      'NETWORK_ERROR',
      (response === null || response === void 0 ? void 0 : response.status) || 503,
      { response, request },
      context
    );
    ...
    switch (classification.type) {
      case 'network':
        return new NetworkError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.response,
          hivemindError.request,
          context
        );
      case 'validation':
        return new ValidationError(
          ...
          hivemindError.field,
          hivemindError.value,
          hivemindError.expected,
          hivemindError.suggestions,
          context
        );
      case 'configuration':
        return new ConfigurationError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.configKey,
          hivemindError.expectedType,
          hivemindError.providedType,
          context
        );
      case 'database':
        return new DatabaseError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.operation,
          hivemindError.table,
          hivemindError.query,
          context
        );
      case 'authentication':
        return new AuthenticationError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.provider,
          hivemindError.reason,
          context
        );
      case 'authorization':
        return new AuthorizationError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.resource,
          hivemindError.action,
          hivemindError.requiredPermission,
          context
        );
      case 'rate-limit':
        return new RateLimitError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.retryAfter || 60,
          hivemindError.limit,
          hivemindError.remaining,
          hivemindError.resetTime,
          context
        );
      case 'timeout':
        return new TimeoutError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.timeoutMs || 30000,
          hivemindError.operation,
          context
        );
      case 'api':
        return new ApiError(
          errors_1.ErrorUtils.getMessage(hivemindError),
          hivemindError.service || 'unknown',
          hivemindError.endpoint,
          errors_1.ErrorUtils.getStatusCode(hivemindError),
          hivemindError.retryAfter,
          context
        );
      default:
