...
export class SwarmCoordinator extends EventEmitter {
  ...
  private decideBroadcast(_messageId: string, botId: string, _channel: string): SwarmDecision {
    const shouldReply = true;
  ...
  private decidePriority(
    ...
    if (existingClaim === undefined) {
      ...
      debug(
        `[SwarmCoordinator] priority: ${botId} → ${shouldReply ? 'reply' : 'skip'} (${reason})`
      );
  ...
  private decideCollaborative(messageId: string, botId: string, channel: string): SwarmDecision {
    ...
    debug(
      `[SwarmCoordinator] collaborative: ${botId} → ${shouldReply ? 'reply' : 'skip'} (${reason})`
    );
