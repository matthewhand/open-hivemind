...
export class DemoModeService {
    ...
    public detectDemoMode(): boolean {
        ...
        const hasNoConfigWarning = warnings.some(w =>
            w.toLowerCase().includes('no bot configuration') ||
            w.toLowerCase().includes('no configuration')
        );

        // Check for critical environment variables
        const hasDiscordToken = process.env.DISCORD_BOT_TOKEN && process.env.DISCORD_BOT_TOKEN.length > 10;
        ...
        this.demoBots = [
            {
                ...
                systemInstruction: 'You are a helpful AI assistant demonstrating the Open-Hivemind platform. Be friendly and informative.',
                status: 'demo',
                connected: true,
                isDemo: true,
                discord: {
            ...
            },
            {
                ...
                systemInstruction: 'You are a friendly assistant helping users understand Open-Hivemind capabilities.',
                status: 'demo',
                connected: true,
                isDemo: true,
                slack: {
    ...
    private getContextualResponses(message: string, _botName: string): string[] {
        ...
        if (lowerMessage.includes('config') || lowerMessage.includes('setup') || lowerMessage.includes('start')) {
            return [
