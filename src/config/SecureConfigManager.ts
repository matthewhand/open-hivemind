...
export class SecureConfigManager {
  ...
  public async getConfig(id: string): Promise<SecureConfig | null> {
    try {
      ...
      // Backwards compatibility migration for removed LLM/Message providers
      if (config.type === 'bot' && config.data) {
        const data = config.data as any;
        const legacyLlmProviders = ['anthropic', 'ollama', 'huggingface', 'local', 'gemini', 'groq'];
        if (data.llmProvider && legacyLlmProviders.includes(data.llmProvider)) {
          data.llmProvider = 'openai';
        }

        const legacyMessageProviders = ['telegram', 'whatsapp'];
        if (data.messageProvider && legacyMessageProviders.includes(data.messageProvider)) {
          data.messageProvider = 'webhook';
        }
      }
