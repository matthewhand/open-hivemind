sed -i 's/expect(result2\.isIdle)/expect(result2.current.isIdle)/g' src/client/src/hooks/__tests__/useInactivity.test.ts
...
sed -i "s/result.current.reset()/act(() => result.current.reset())/g" src/client/src/hooks/__tests__/useInactivity.test.ts
