...
import { Migration, MigrationManager } from '../../src/database/MigrationManager';
...
describe('MigrationManager', () => {
  ...
  describe('Executed Migrations Retrieval', () => {
    it('should retrieve executed migrations', async () => {
      const mockMigrations = [{ id: '001_test_migration' }, { id: '002_test_migration' }];
      mockDb.all.mockResolvedValueOnce(mockMigrations);

      const executed = await migrationManager.getExecutedMigrations();

      expect(mockDb.all).toHaveBeenCalledWith('SELECT id FROM migrations ORDER BY version');
