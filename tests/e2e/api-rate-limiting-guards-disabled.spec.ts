...
test.describe('API Rate Limiting UI', () => {
  test('Capture Rate Limiting Section Guards Disabled', async ({ page }) => {
    ...
    await rateLimiterToggle.uncheck();

    await page.waitForTimeout(500);

    // Screenshot the collapse content
    await page.locator('.collapse:has(.collapse-title:has-text("Rate Limiter"))').screenshot({ path: 'docs/screenshots/api-rate-limiting-guards-disabled-before.png' });
  });
});
