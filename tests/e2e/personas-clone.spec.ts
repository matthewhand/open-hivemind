...
test.describe('Personas Cloning', () => {
  ...
  test('can clone a custom persona', async ({ page }) => {
    ...
    try {
      ...
      const newCard = page.locator(`[data-testid="persona-card"]:has-text("${uniqueName}")`).first();
      ...
      const cloneDescInput = modal.locator('input[placeholder="Short description of this persona"]');
      ...
      const clonedCard = page.locator(`[data-testid="persona-card"]:has-text("Copy of ${uniqueName}")`).first();
