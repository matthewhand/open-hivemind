...
describe('slackConfig', () => {
  ...
  it('should fall back gracefully with invalid mode', () => {
    const OLD_ENV = process.env;
    ...
    // Should gracefully fall back from invalid mode to default mode with Zod safeParse
    const config = require('../../src/config/slackConfig').default;
    expect(config.get('SLACK_MODE')).toBe('socket'); // Default value
