...
describe('Live Discord Integration', () => {
  test('Discord service loads in dev environment', async () => {
    ...
    const { stdout } = await execAsync(
      'cd ' +
        process.cwd() +
        ' && cross-env DISCORD_BOT_TOKEN="mock" MESSAGE_PROVIDER="discord" timeout 30s npm run dev 2>&1 | grep -E "(Messenger services loaded|Filter results|Bot started)" || true'
    );
