...
describe('Live Discord Integration', () => {
  test('Discord service loads in dev environment', async () => {
    ...
    const { stdout } = await execAsync(
      'timeout 30s npx cross-env NODE_ENV=development MESSAGE_PROVIDER=discord DISCORD_BOT_TOKEN=test DOTENV_CONFIG_PATH=.env node -r dotenv/config --import tsx src/index.ts 2>&1 | grep -E "(Messenger services loaded|Filter results|Bot started)" || true'
    );
