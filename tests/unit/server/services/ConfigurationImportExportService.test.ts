...
describe('ConfigurationImportExportService - Backup Retention', () => {
  ...
  test('happy path: should prune oldest backups when exceeding limit', async () => {
    // The backup logic is delegated to the internal backupManager.
    // Mock the backupManager's listBackups and deleteBackup methods directly.
    const backupManager = (service as any).backupManager;

    backupManager.listBackups = jest.fn().mockResolvedValue([
    ...
    backupManager.deleteBackup = jest.fn().mockResolvedValue(true);
    ...
    // listBackups is called AFTER the new backup metadata is written.
    // listBackups returns 4 items. The limit is 3. So it should slice(3) -> 1 item (the oldest)
    expect(backupManager.deleteBackup).toHaveBeenCalledTimes(1);
    expect(backupManager.deleteBackup).toHaveBeenCalledWith('b1');
  ...
  test('error case: should swallow errors during retention check and still return success', async () => {
    const backupManager = (service as any).backupManager;
    backupManager.listBackups = jest.fn().mockRejectedValue(new Error('File system error'));
