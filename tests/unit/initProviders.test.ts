...
import { loadPlugin, instantiateMemoryProvider, instantiateToolProvider } from '../../src/plugins/PluginLoader';
...
const mockedLoadMemoryProfiles = loadMemoryProfiles as jest.MockedFunction<typeof loadMemoryProfiles>;
...
const mockedInstantiateMemoryProvider = instantiateMemoryProvider as jest.MockedFunction<typeof instantiateMemoryProvider>;
const mockedInstantiateToolProvider = instantiateToolProvider as jest.MockedFunction<typeof instantiateToolProvider>;
...
describe('initProviders — memory and tool discovery', () => {
  beforeEach(() => {
    ...
    (mockRegistry.getMemoryProvider as jest.Mock).mockImplementation(
      (name: string) => mockRegistry._memoryProviders.get(name)
    );
    (mockRegistry.getToolProvider as jest.Mock).mockImplementation(
      (name: string) => mockRegistry._toolProviders.get(name)
    );
  ...
  describe('discoverMemoryProviders', () => {
    test('discovers memory providers from profiles', async () => {
      ...
      expect(mockRegistry.registerMemoryProvider).toHaveBeenCalledWith('mem0-default', mockInstance);
    });
    ...
    test('discovers memory providers from packages/memory-* directories', async () => {
      ...
      mockedLoadPlugin
        .mockReturnValueOnce(mockMemMod)
        .mockReturnValueOnce(mockToolMod);
      ...
      expect(mockRegistry.registerMemoryProvider).toHaveBeenCalledWith('memory-zep', mockMemInstance);
    });
    ...
    test('handles failed memory provider instantiation gracefully', async () => {
      mockedLoadMemoryProfiles.mockReturnValue({
        memory: [
          { key: 'bad-mem', name: 'Bad', provider: 'memory-bad', config: {} },
        ],
      });
      ...
      await expect(initProviders()).resolves.not.toThrow();
      expect(mockRegistry.registerMemoryProvider).not.toHaveBeenCalledWith('bad-mem', expect.anything());
    });

    test('prefixes provider name with memory- if not already prefixed', async () => {
      mockedLoadMemoryProfiles.mockReturnValue({
        memory: [
          { key: 'mem0-short', name: 'Mem0 Short', provider: 'mem0', config: {} },
        ],
      });
  ...
  describe('discoverToolProviders', () => {
    test('discovers tool providers from profiles', async () => {
      mockedLoadToolProfiles.mockReturnValue({
        tool: [
          { key: 'mcp-github', name: 'GitHub MCP', provider: 'tool-mcp-github', config: { token: 't' } },
        ],
      });
    ...
    test('discovers tool providers from packages/tool-* directories', async () => {
      setupPackagesDir([
        { name: 'tool-code-exec', isDirectory: () => true },
      ]);
      ...
      expect(mockRegistry.registerToolProvider).toHaveBeenCalledWith('tool-code-exec', mockInstance);
    });

    test('handles failed tool provider instantiation gracefully', async () => {
      mockedLoadToolProfiles.mockReturnValue({
        tool: [
          { key: 'bad-tool', name: 'Bad', provider: 'tool-bad', config: {} },
        ],
      });
      ...
      await expect(initProviders()).resolves.not.toThrow();
      expect(mockRegistry.registerToolProvider).not.toHaveBeenCalledWith('bad-tool', expect.anything());
    });

    test('prefixes provider name with tool- if not already prefixed', async () => {
      mockedLoadToolProfiles.mockReturnValue({
        tool: [
          { key: 'mcp-short', name: 'MCP Short', provider: 'mcp', config: {} },
        ],
      });
  ...
  describe('profiles and package discovery integration', () => {
    test('loads profiles and instantiates providers from both config and packages', async () => {
      ...
      mockedLoadToolProfiles.mockReturnValue({
        tool: [
          { key: 'mcp-cfg', name: 'MCP Config', provider: 'tool-mcp', config: { t: 'x' } },
        ],
      });
      ...
      mockedInstantiateMemoryProvider
        .mockReturnValueOnce(mem0Inst)
        .mockReturnValueOnce(zepInst);
      mockedInstantiateToolProvider
        .mockReturnValueOnce(mcpInst)
        .mockReturnValueOnce(searchInst);
    ...
    test('skips package-based discovery if already registered via profile', async () => {
      mockedLoadMemoryProfiles.mockReturnValue({
        memory: [
          { key: 'memory-zep', name: 'Zep', provider: 'memory-zep', config: {} },
        ],
      });
