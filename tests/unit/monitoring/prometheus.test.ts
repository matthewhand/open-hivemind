...
describe('ProviderMetricsCollector - Prometheus Export', () => {
  ...
  it('should format message provider metrics with correct MetricNames', () => {
    ...
    expect(prometheusFormat).toMatch(
      new RegExp(`^# HELP ${MetricNames.MESSAGES_RECEIVED} Total messages received$`, 'm')
    );
    expect(prometheusFormat).toMatch(
      new RegExp(`^# TYPE ${MetricNames.MESSAGES_RECEIVED} counter$`, 'm')
    );
    expect(prometheusFormat).toMatch(
      new RegExp(`^${MetricNames.MESSAGES_RECEIVED}{provider="discord"} 1 \\d+$`, 'm')
    );
  });
  ...
  it('should format LLM provider metrics with correct MetricNames', () => {
    ...
    // Verify correct format
    expect(prometheusFormat).toMatch(
      new RegExp(`^# HELP ${MetricNames.LLM_TOKENS_USED} Total tokens used$`, 'm')
    );
    expect(prometheusFormat).toMatch(
      new RegExp(`^# TYPE ${MetricNames.LLM_TOKENS_USED} counter$`, 'm')
    );
    expect(prometheusFormat).toMatch(
      new RegExp(`^${MetricNames.LLM_TOKENS_USED}{provider="openai"} 30 \\d+$`, 'm')
    );
  });
});
