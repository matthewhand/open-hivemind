...
describe('SlackBotManager', () => {
  ...
  it.skip('should handle initialization, configuration, and message handling', async () => {
    ...
    const failManager = new SlackBotManager(
    ...
    const handlerManager = new SlackBotManager(
      [{ token: 'botToken1', signingSecret: 'secret1' }],
      'socket'
    );
    ...
    const mockMessage = {
      getText: () => 'test',
      data: { event_ts: '123', ts: '123', channel: 'C123', user: 'U456' },
    };
    ...
    const noHandlerManager = new SlackBotManager(
      [{ token: 'botToken1', signingSecret: 'secret1' }],
      'socket'
    );
