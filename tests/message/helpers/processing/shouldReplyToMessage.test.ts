...
import { shouldReplyToUnsolicitedMessage, looksLikeOpportunity } from '../../../../src/message/helpers/unsolicitedMessageHandler';
...
describe('shouldReplyToMessage', () => {
    ...
    it('should always reply when message is a reply-to-bot', async () => {
    ...
    it('should treat bot name in text as spoken-to when MESSAGE_ONLY_WHEN_SPOKEN_TO=true', async () => {
        ...
        expect((await shouldReplyToMessage(mockMessage, 'bot-id', 'discord', 'MyBot')).shouldReply).toBe(true);
    });
    ...
    it('should treat unicode punctuation around bot name as spoken-to', async () => {
        ...
        expect((await shouldReplyToMessage(mockMessage, 'bot-id', 'discord', 'seneca')).shouldReply).toBe(true);
    ...
    it('should accept a list of candidate bot names', async () => {
        ...
        expect((await shouldReplyToMessage(mockMessage, 'bot-id', 'discord', ['MyBot', 'Seneca'])).shouldReply).toBe(true);
    });
    ...
    it('should allow unaddressed replies within the MESSAGE_ONLY_WHEN_SPOKEN_TO grace window', async () => {
        ...
        mockMessage.getText.mockReturnValue('help please');
        jest.spyOn(Math, 'random').mockReturnValue(0.0);
        expect((await shouldReplyToMessage(mockMessage, 'bot-id', 'discord', 'SomeOtherName')).shouldReply).toBe(true);

        global.Date.now = RealDate.now;
    });

    it('should allow unaddressed bot messages within the grace window (floodgates stay open)', async () => {
        (messageConfig.get as jest.Mock).mockImplementation((key) => {
            if (key === 'MESSAGE_WAKEWORDS') return ['hey bot', 'bot'];
            if (key === 'MESSAGE_ONLY_WHEN_SPOKEN_TO') return true;
            if (key === 'MESSAGE_ONLY_WHEN_SPOKEN_TO_GRACE_WINDOW_MS') return 300000;
    ...
    it('should not reply to unaddressed bot messages when not within grace and MESSAGE_ALLOW_BOT_TO_BOT_UNADDRESSED=false', async () => {
        ...
        expect((await shouldReplyToMessage(mockMessage, 'bot-id', 'discord', 'MyBot')).shouldReply).toBe(false);
    });
});
