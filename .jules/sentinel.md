...
**Vulnerability:** The `validateRepoUrl` function in `PluginManager.ts` checked the decoded URL using `decodeURI()`. However, `decodeURI` leaves certain characters (like `%20` or `%3B`) encoded. This allowed attackers to bypass validation checks (e.g., `.includes(" ")`) by encoding those characters in the payload.
**Learning:** Using `decodeURI` for security checks against specific characters is insufficient because it does not decode all URL components. `decodeURIComponent` decodes all parts, but it can throw a `URIError` if given an invalid sequence.
**Prevention:** Always use `decodeURIComponent()` instead of `decodeURI()` for security checks, and always wrap `decodeURIComponent()` in a `try-catch` block to handle potential `URIError` exceptions resulting from malformed sequences, preventing Denial of Service (DoS) crashes.