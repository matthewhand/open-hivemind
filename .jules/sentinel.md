...
## 2025-05-18 - Path Traversal in Import/Export Backups
**Vulnerability:** A path traversal vulnerability existed in `src/server/routes/importExport.ts` and `src/server/services/ConfigurationImportExportService.ts` where user inputs (`name` and `backup.name`) were used directly to construct file paths for backups without validation.
**Learning:** Using user-provided strings directly in `path.join` to read, write, or delete files opens up the system to path traversal vulnerabilities if an attacker includes `../` payloads.
**Prevention:** 1. Sanitize user input by replacing potentially dangerous characters using a regex (e.g., `replace(/[^a-zA-Z0-9_-]/g, '_')`). 2. Verify resolved paths stay within intended base directories by using `path.resolve(path).startsWith(path.resolve(baseDir) + path.sep)`.
