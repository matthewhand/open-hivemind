...
## 2025-03-20 - Letta API SSRF Protection

**Vulnerability:** The Letta API integration routes (`/api/letta/agents` and `/api/letta/agents/:id`) accepted a user-controlled `apiUrl` parameter and passed it directly to the Letta client without validation, allowing Server-Side Request Forgery (SSRF) against internal endpoints.
**Learning:** Even when using dedicated SDK clients (like `@letta-ai/letta-client`), passing unvalidated, dynamic endpoints controlled by headers or query parameters opens the service up to SSRF.
**Prevention:** Always validate external, dynamic URLs. Use the centralized `isSafeUrl` guard to block internal IP resolution and strictly validate domain names against an allowlist (e.g., `letta.com` and its subdomains) while providing a controlled environment escape hatch (e.g., `LETTA_ALLOW_LOCAL`) for deliberate local testing.
