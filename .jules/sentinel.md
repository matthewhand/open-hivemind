...
## 2025-03-24 - Cryptographically Weak ID Generation
**Vulnerability:** `Math.random().toString(36).substr(2, 9)` was used to generate unique IDs across the application, including for Bot Instances and Providers in `src/client/src/hooks/useBots.ts`, `src/client/src/hooks/useBotProviders.ts`, and `src/client/src/contexts/BotContext.tsx`.
**Learning:** `Math.random()` is a cryptographically weak pseudo-random number generator (PRNG) that produces predictable values. Using it for ID generation can lead to ID enumeration or collision vulnerabilities, especially in security-sensitive contexts or multi-tenant applications.
**Prevention:** Always use `crypto.randomUUID()` or a similarly secure random generation method from the built-in Node.js/Browser `crypto` module to generate unique IDs and tokens to prevent predictability and collisions.
