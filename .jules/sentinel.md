...
## 2025-05-18 - Shell Command SSRF in Plugin Installation
**Vulnerability:** A shell command Server-Side Request Forgery (SSRF) vulnerability existed where user-supplied URLs (e.g., `repoUrl` for plugin installation) were passed directly to system shell execution functions like `exec` and `execFileSync` (e.g., in `git clone`) without prior SSRF validation.
**Learning:** Outbound network operations executed via system shell processes bypass standard Node.js HTTP protections (`fetch`, `axios`). As a result, standard SSRF protections applied only to HTTP clients fail to protect shell-based network calls.
**Prevention:** Outbound shell commands that accept URLs (such as `git clone`) must validate these URLs explicitly using `isSafeUrl` before passing them to execution functions.
