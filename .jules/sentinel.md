...
## 2026-03-28 - Prevent Path Traversal in ImportExportService
**Vulnerability:** The `backupId` from the route was being used to fetch a backup object, and the object's `name` was used to construct a file path for restore and download without sanitization. An attacker who could create a backup with a malicious name (e.g., `../../../etc/passwd`) could potentially trigger a path traversal when that backup was later restored or downloaded.
**Learning:** Even if an ID (like `backupId`) is used to look up an object, any properties of that object (like `backup.name`) used in file path construction must still be sanitized.
**Prevention:** Introduce a centralized path resolution method (`getBackupFilePath`) that uses `path.basename()` on the user-provided parts of the filename, resolves the absolute path, and verifies it starts with the expected base directory using `startsWith`.
