...
## 2025-03-12 - XSS via dangerouslySetInnerHTML and JSON.stringify
**Vulnerability:** The `ToolResultModal` component used `dangerouslySetInnerHTML` to render syntax-highlighted JSON strings. The comments wrongly assumed that `JSON.stringify()` escapes HTML characters like `<` and `>`.
**Learning:** `JSON.stringify()` DOES NOT escape HTML entities. An attacker controlling the JSON payload (e.g., via a malicious MCP tool result or user input reflected in arguments) can inject HTML tags like `<script>alert(1)</script>` into the DOM.
**Prevention:** Never assume `JSON.stringify` provides XSS protection. When rendering JSON with `dangerouslySetInnerHTML` (for instance, to support custom formatting tags), explicitly escape HTML entities (`<`, `>`, `&`) from the raw JSON string before applying custom HTML transformations.
