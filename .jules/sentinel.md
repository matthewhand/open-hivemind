...
## 2026-03-19 - Replaced Math.random with crypto.randomBytes
**Vulnerability:** Use of insecure Math.random() for sensitive ID generation.
**Learning:** Math.random() is a weak pseudorandom number generator (PRNG) and produces predictable output. It was used in several places to generate IDs, such as audit logs, synthetic messages, anomalies, conversations, configuration change and rollback snapshot IDs.
**Prevention:** Replaced Math.random() uses with crypto.randomBytes(8).toString('hex') where cryptographically strong random data is required. Use UUID for ID generation.
